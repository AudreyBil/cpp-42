...
int main(void) {
    ...
    std::cout << CYAN << BOLD << "---Test existing int---" << RESET
              << std::endl;
    try {
        it = ::easyfind(arr, 4);
    ...
    try {
        itDeq = ::easyfind(myDeq, 5);
        std::cout << *itDeq << " was found\n";
    } catch (std::exception &e) {
        std::cout << e.what() << std::endl;
    }
