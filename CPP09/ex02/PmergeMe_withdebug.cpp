...
void PmergeMe::insertPendChainVec(std::vector<Pair> pairs, std::vector<int>& mainChain, std::vector<int>pendChain)
{
	...
	if (prevIndex < pendChain.size() -1) {
		for (size_t j = pendChain.size() - 1; j > prevIndex; --j) {
			int highPair = findPairing(const_cast<std::vector<Pair>&>(pairs), pendChain[j]);
			insertMember(mainChain, pendChain[j], highPair);
		}
	}
}
